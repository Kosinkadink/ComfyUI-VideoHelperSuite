    ...
    if isinstance(entry, dict):
        ...
        for k in entry:
            ...
            if collapse_single:
                ...
                name = k
            if entry.get("collapsed", False) or collapse_single:
                html += f'<div vhs_title={name} style=\"display: flex; font-size: {size}em\" class=\"VHS_collapse\"><div style=\"color: #AAA; height: 1.5em\">[+]</div><div style=\"overflow-y: hidden; height: 1.5em; color: #CCC;\">{name}: {as_html(entry[k], depth=depth+1)}</div></div>'
            else:
                html += f'<div vhs_title={name} style=\"display: flex; font-size: {size}em\" class=\"VHS_collapse\"><div style=\"color: #AAA; height: 1.5em\">[-]</div><div>{name}: {as_html(entry[k], depth=depth+1)}</div></div>'
        return html
